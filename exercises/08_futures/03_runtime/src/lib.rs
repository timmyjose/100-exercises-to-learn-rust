    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(fixed_reply_impl(first, reply.clone()));
    let handle2 = tokio::spawn(fixed_reply_impl(second, reply.clone()));

    let _ = tokio::join!(handle1, handle2);
}

async fn fixed_reply_impl<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
